...
const reversedString = (str) => {
  const string = str.split("").reduce((acc, curr) => curr + acc, "")
  return string
};
...
const calculateAverage = (arr) => {
  const avg = arr.reduce((acc, curr) => {
    return {
      count: ++acc.count, sum: (acc.sum += curr)
    }
  }, {count: 0, sum: 0})
  const total = avg.sum / avg.count;
  return total;
};
